...
console.log('tweetTypes:', tweetTypes)
...
export const audienceValidator = wrapRequestHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (tweet.audience === TweetAudience.TwitterCircle) {
    ...
    //Kiểm tra tài khoản của user đăng tweet có bị khóa hay bị xóa chưa
    ...
    //Kiểm tra người xem tweet này có nằm trong danh sách Circle của tác giả hay không
      ...
      limit: {
        ...
        custom: {
          options: (value, { req }) => {
            ...
            if (num < 1 || num > 100) {
              throw new Error(TWEETS_MESSAGES.PAGE_SIZE_MAXIMUM_IS_100_AND_MINIMAL_IS_ONE)
            }
            return true
          }
        }
      ...
      page: {
        ...
        custom: {
          options: (value, { req }) => {
            const num = Number(value)
            if (num < 1) {
              throw new Error(TWEETS_MESSAGES.PAGE_IS_ALWAYS_GREATER_THAN_OR_EQUAL_TO_ONE)
            }
            return true
          }
        }
